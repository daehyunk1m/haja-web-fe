...
**Scope**: types, utils, lib, shared, hooks, contexts, components, pages, app, docs, config, test
...
- 단순 수정 (1~2 파일, 30줄 이하): main에서 직접 작업 가능

### 정리

- feature 완료 후 main에 merge된 브랜치는 삭제를 제안한다
...
- **모드**: auto  (off | confirm | auto)
- **커밋 후 푸시**: 아니오 (push는 수동)
...
3. `yarn build && yarn test:run && yarn lint` 통과 후 커밋 (off 모드면 제안)
