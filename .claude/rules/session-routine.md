...
| Post-Green | Security Reviewer | `agents/security-reviewer.md` | feature.category가 auth, security, api일 때 / (infra 트랙) 변경이 보안 표면(.env·secrets·auth/세션/토큰·provider 배선·CORS·쿠키)에 닿을 때 |
...
| On-demand | Debugger | `agents/debugger.md` | validate 3회 실패 시 / E2E 실패 시 / 인프라 트랙 통합 검증(빌드·부팅·실동작) 실패 시 |
...
5. **통합 검증 (필수 — RED→GREEN 대체 게이트)**:
   - `yarn build && yarn test:run && yarn lint`(빌드+타입+린트+테스트)가 통과해야 한다.
...
7. **Security Reviewer (보안 표면이면 필수)**: `feature.category`가 auth, security, api이거나 **변경이 보안 표면에 닿으면 category와 무관하게(infra/config 포함) 반드시 실행**한다. **보안 표면**(기계적 판정 — 변경 경로/내용이 다음을 포함): 인증/세션/토큰 · `.env`/secrets/자격증명 · auth provider·미들웨어 배선 · CORS · 쿠키 · 권한. 모호하면 실행한다(디폴트 실행 — '모호 → TDD'와 대칭). 인프라라고 보안 리뷰를 건너뛰지 않는다(이슈 #6의 AuthProvider 배선이 정확히 이 경우다).
...
다음 중 하나면 실행한다:
- feature.category가 auth, security, api 중 하나일 때, 또는
...
**실행**: 해당 feature 스펙만 선택 실행한다(전체 스위트 아님) — E2E 러너 `yarn test:e2e`에 `--grep @feature:{featureID}`를 적용한다(유닛 러너 `yarn test:run`가 **아님** — `.e2e.ts`는 유닛 테스트 글롭에 수집되지 않아 유닛 러너로는 0개 실행되어 거짓 PASS가 된다). (전체 @critical 게이팅은 pre-push 훅의 책임.)

**분기**:
- PASS → 기능 완료 처리.
- FAIL(로직) → 마찰 이벤트 기록(`e2e-fail`) — § 마찰 로그 참조(.harness-friction.jsonl에 append) → Phase 3 (Green) 재진입, **시도 횟수 누적**(`3` 한도 — NEEDS_FIX와 동일).
