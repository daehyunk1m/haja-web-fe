...
### 6.1 메인 화면 (일별 태스크 컨테이너 + Someday 컨테이너)

- 서비스 접속 시 **오늘 날짜**의 태스크 컨테이너가 기본 표시
- 캘린더(`CalendarContainer`)로 날짜 선택 가능 (`useDateStore` 연동)
- 접속 시점에 Lazy Migration 실행
- **Someday 컨테이너**: 날짜와 무관한 태스크(`type: "someday"`)를 별도 컨테이너로 표시. 일별 컨테이너와 분리되어 항상 접근 가능
...
- 태스크 타입 선택 가능: `"task"` (오늘 할 일) / `"someday"` (언젠가 할 일)
  - `task`: 날짜에 종속 — 해당 날짜의 일별 컨테이너에 표시, 자동 이관 대상
  - `someday`: 날짜와 무관 — Someday 컨테이너에 표시, 자동 이관 대상에서 제외
  - 생성 후에도 `someday ↔ task` 타입 변경 가능
...
- 기본: 생성순
- 드래그로 순서 변경: ✅ 구현 완료 (F010, @dnd-kit 기반)
...
| ~~`order` 속성~~ | ~~드래그 순서 변경을 위한 정렬 순서 필드~~ | **구현 완료** (F010, taskOrder로 관리) |
...
| F010 | 드래그 순서 변경 | ✅ 완료 |
| F011 | Someday 컨테이너 (날짜 무관 태스크 분리) | ❌ 미구현 |
...
| v0.3 | 2026-04-10 | F010 완료 반영, F011(Someday 컨테이너) 추가, task/someday 타입 분리 명세 보강 |
